...
const Username = async ({ params, searchParams }) => {
  ...
  try {
    ...
    // Filter completed payments for stats
    const completedPayments = payments.filter(payment => payment.done);
    ...
    if (completedPayments.length > 0) {
      ...
      const topSupporters = completedPayments.slice(0, 10).map(payment => ({
        name: payment.name,
