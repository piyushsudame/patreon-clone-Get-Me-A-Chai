...
export async function POST(request) {
  try {
    ...
    // Use user's Stripe secret key if available, otherwise fall back to the global one
    const stripeSecretKey = user.stripeSecretId && user.stripeSecretId.trim() 
      ? user.stripeSecretId.trim() 
      : process.env.STRIPE_SECRET_KEY;
    
    // Check if Stripe key is available
    if (!stripeSecretKey) {
      console.error('Stripe secret key is missing for user:', username);
      return NextResponse.json(
        { error: 'Stripe configuration error - No Stripe key available' },
        { status: 500 }
      );
    }
    ...
    // User was already found in the code above
    if (!user) {
      return NextResponse.json(
        { error: 'User not found' },
        { status: 404 }
