...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      const fetchUserData = async () => {
      if (session?.user?.email?.emai        try {
          ...
          if (response.ok) {
            ...
            if (userData.user) {
        l) {
        try         name: userData.user.name || userData.user.username || session.user.name || '',,
                profilePicture: userData.user.profilepic || '',
                coverPicture: userData.user.coverpic || '',
                stripePublishableId: userData.user.stripePublishableId || prevData.stripePublishableId || '',
                stripeSecretId: userData.user.stripeSecretId || prevData.stripeSecretId || ''
              }));
              return;
            }
          }
    ...
    if (session?.user) {
      fetchUserData();                  email: userData.user.email || Fetch user data from the database
          const response = await fetch(`/api/get-user?email=${encodeURIComponent(session.user.email)}`);
