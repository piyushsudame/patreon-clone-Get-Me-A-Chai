  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        let userPublishableKey = null;
        try {
          const userResponse = await fetch(`/api/get-user?username=${username}`);
          if (userResponse.ok) {
            const userData = await userResponse.json();
            // Use the user's publishable key if available
            userPublishableKey = userData.user?.stripePublishableId || null;
          } else {
            console.warn('Failed to fetch user data for Stripe key, using default key');
          }
        } catch (error) {
          console.error('Error fetching user data for Stripe key:', error);
        }
        
        // Initialize Stripe with the user's key or fall back to the global one
